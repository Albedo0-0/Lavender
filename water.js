...
const Water = (function () {
  ...
  function pickerHtml(title) {
    const buttons = Object.keys(WaterData.CATEGORIES).map(function (key) {
      return '<button class="water-cat-btn btn btn-secondary" data-cat="' + key + '">' + WaterData.CATEGORIES[key].label + '</button>';
    }).join('');
  ...
  function showReminderPrompt() {
    Modal.open(
      '<h3 class="section-heading">Hydration check-in</h3>' +
      '<p>How much water have you had?</p>' +
      '<button id="water-reminder-done" class="btn btn-primary">Done</button> ' +
      '<button id="water-reminder-later" class="btn btn-secondary">Later</button>'
    );
