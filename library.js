...
const Library = (function () {
  ...
  function renderChapters(container) {
    ...
    const tagChipsHtml = allFilterTags.length === 0 ? '' :
      '<div class="library-filter-chips" id="library-filter-chips">' +
        allFilterTags.map(function (tag) {
          return '<button class="library-filter-chip" data-tag-id="' + tag.tagId + '" style="border-color:' + tag.color + '">' + esc(tag.name) + '</button>';
        }).join('') +
      '</div>';
    ...
    if (topics.length === 0) {
      ...
      html += '<div class="planner-history-chapter-list" id="library-chapter-list">' + topics.map(function (t) {
        ...
        return '<div class="planner-history-chapter-item" data-name="' + t.topicName.toLowerCase() + '" data-tags="' + (t.tags || []).join(',') + '">' +
          dot +
  ...
  function renderChapter(container) {
    ...
    const tagsHtml = '<div class="library-topic-tags">' +
      tagIds.map(function (tagId) {
        ...
        if (!tag) return '';
        return '<span class="library-tag" style="border-color:' + tag.color + ';color:' + tag.color + '">' + esc(tag.name) + '</span>';
      }).join('') +
      '<button id="library-tag-manage-btn">Manage tags</button>' +
    '</div>';
    ...
    container.innerHTML =
      ...
      '<div class="library-topic-actions">' +
        ...
        '<button id="library-action-color">Color</button>' +
  ...
  function renderRightPanel() {
    ...
    content.innerHTML = tasks.map(function (t) {
      ...
      const reschedBtn = (tab === 'pending')
        ? '<button class="library-reschedule-task-btn" data-task-id="' + t.taskId + '">Reschedule</button>'
        : '';
