...
const Library = (function () {
  ...
  function renderSubjects(container) {
    ...
    if (favTopics.length > 0) {
      html += '<div class="planner-history-favorites">' +
        '<div class="planner-suggested-group-title section-heading">Favorites</div>' +
        favTopics.map(function (t) {
          return '<button class="planner-history-fav-btn btn btn-secondary" data-subject="' + t.subject + '" data-topic-id="' + t.topicId + '">' + t.subject + ' \u00B7 ' + t.topicName + '</button>';
        }).join('') +
      '</div>';
    }
  ...
  function renderChapters(container) {
    ...
    let html = '<button class="planner-history-back btn btn-secondary">&lt; Subjects</button>' +
      '<h4 class="planner-history-heading section-heading">' + activeSubject + '</h4>' +
      '<input type="text" id="library-search" class="input" placeholder="Search chapters...">' +
      tagChipsHtml +
      '<button id="library-add-topic-btn" class="btn btn-primary">Add Topic</button>';

    if (topics.length === 0) {
      html += '<p class="planner-empty empty-state">No chapters yet for ' + activeSubject + '.</p>';
    } else {
      html += '<div class="planner-history-chapter-list list-row-group" id="library-chapter-list">' + topics.map(function (t) {
        ...
        return '<div class="planner-history-chapter-item list-row" data-name="' + t.topicName.toLowerCase() + '" data-tags="' + (t.tags || []).join(',') + '">' +
          dot +
  ...
  function renderChapter(container) {
    ...
    const tagsHtml = '<div class="library-topic-tags chip-row">' +
      ...
      '<button id="library-tag-manage-btn" class="btn btn-secondary">Manage tags</button>' +
    '</div>';

    container.innerHTML =
      '<button class="planner-history-back btn btn-secondary">&lt; ' + activeSubject + '</button>' +
      ...
      '<div class="library-topic-actions">' +
        ...
        '<button id="library-action-history" class="btn btn-secondary">History</button>' +
        '<button id="library-action-upcoming" class="btn btn-secondary">Upcoming</button>' +
        '<button id="library-action-links" class="btn btn-secondary">Links</button>' +
        '<button id="library-action-target" class="btn btn-secondary">Target</button>' +
        '<button id="library-action-edit" class="btn btn-secondary">Edit</button>' +
        '<button id="library-action-color" class="btn btn-secondary">Color</button>' +
        '<button id="library-action-remove" class="btn btn-danger">Remove</button>' +
      '</div>';
    ...
    document.getElementById('library-action-color').addEventListener('click', function () {
      const meta = PlannerData.getAllTopics()[activeTopicId] || {};
      const input = document.createElement('input');
      input.type = 'color';
      input.value = meta.color || '#b39ddb';
      input.addEventListener('change', function () {
        PlannerData.updateTopicMeta(activeTopicId, { color: input.value });
        render();
      });
      input.click();
    });
    ...
    document.getElementById('library-tag-manage-btn').addEventListener('click', function () {
      Tags.openManagerForTopic(activeTopicId);
    });
  }
  ...
  function openUpcomingModal(topicId) {
    ...
    const html = tasks.length === 0
      ...
      : tasks.map(function (t) {
          return '<div class="planner-task-row list-row"><div class="planner-task-meta">' + PlannerData.taskLabel(t) + '</div>' +
