...
const Ladybug = (function () {
  ...
  const BOTTOM_RETURN_MS_MIN = 1600;   // wait offscreen (bottom) before re-entering from the top
  const BOTTOM_RETURN_MS_MAX = 3200;
  const TAB_TRAVERSE_MS_MIN = 450;     // wait offscreen (right) before entering the next tab from the left
  const TAB_TRAVERSE_MS_MAX = 900;
  ...
  function mount(container, options) {
    ...
    let exitEdge = null;           // which edge the current 'exit' is heading through
    ...
    // Next tab in the existing nav-button order (wraps after the last one).
    function nextScreenName() {
      const order = [];
      document.querySelectorAll('#main-nav .nav-btn').forEach(function (btn) {
        if (btn.dataset && btn.dataset.screen) order.push(btn.dataset.screen);
      });
      if (order.length < 2) return null;
      let current = null;
      const activeBtn = document.querySelector('#main-nav .nav-btn.active');
      if (activeBtn && activeBtn.dataset) current = activeBtn.dataset.screen;
      if (!current && typeof State !== 'undefined' && State.get) current = State.get().currentScreen;
    ...
    function enterOffscreenWait() {
      ...
      const now = performance.now();
      if (exitEdge === 'bottom') {
        // Fell out the bottom: come back in through the top shortly.
        modeUntil = now + MiscCore.rand(BOTTOM_RETURN_MS_MIN, BOTTOM_RETURN_MS_MAX);
      } else if (exitEdge === 'right' && nextScreenName()) {
        // Fully offscreen to the right: hop to the next tab while unseen,
        // then she flies in from that tab's left edge.
        const next = nextScreenName();
        try {
          if (typeof Nav !== 'undefined' && Nav.switchTo) {
            Nav.switchTo(next);
            modeUntil = now + MiscCore.rand(TAB_TRAVERSE_MS_MIN, TAB_TRAVERSE_MS_MAX);
          } else {
            exitEdge = null;
            modeUntil = now + MiscCore.rand(6000, 16000);
          }
        } catch (err) {
          exitEdge = null;
          modeUntil = now + MiscCore.rand(6000, 16000);
    ...
    function enterWrappedEntry(fromEdge) {
      // Same creature, same facing, same petal — just re-emerging on the
      // opposite side she left from.
      mode = 'enter';
      clearClasses();
      el.classList.add('is-flying', 'is-airborne');
      if (fromEdge === 'bottom') {
        x = MiscCore.clamp(x, 4, vw() - 4);
        y = -20;
        wanderTargetX = x; wanderTargetY = 30;
      } else {
        y = MiscCore.clamp(y, 4, vh() - 4);
        x = -24;
        wanderTargetX = 30; wanderTargetY = y;
      }
      setPos();
    ...
    function tick(now) {
      ...
      switch (mode) {
        ...
        case 'offscreen': {
          if (now >= modeUntil) {
            const edgeUsed = exitEdge;
            exitEdge = null;
            if (edgeUsed === 'bottom' || edgeUsed === 'right') enterWrappedEntry(edgeUsed);
            else enterReturnEdge();
          }
          break;
        }
