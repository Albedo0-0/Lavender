 ...
 * the wish is released, not tracked. This module has no reward system
 * and no EXP/productivity effect. It does read/write one small piece
 * of State (miscWishButterfly: { date, appeared }) purely to track
 * whether it has already made its one appearance for the calendar
 * day (Midnight-to-Midnight Daily State) — never the wish text itself.
 *
 * Usage:
...
const WishButterfly = (function () {
  ...
  // Midnight-to-Midnight Daily State (new): the butterfly is meant to appear at most once per
  // calendar day, not once per app open/close and not on a recurring in-session timer. State is
  // persisted (mirrors WaterData.getReminderState()'s per-date pattern), so it survives a reload,
  // and re-arms only via TimeEngine's own rollover signal (Section 4/38 — no second clock/reset
  // system) rather than a local setInterval.
  function pad(n) { return n < 10 ? '0' + n : '' + n; }
  function todayStr() {
    const t = new Date();
    return t.getFullYear() + '-' + pad(t.getMonth() + 1) + '-' + pad(t.getDate());
  }
  function hasAppearedToday() {
  ...
  function mount(container, options) {
    ...
    function spawn() {
      ...
      function end() {
        ...
        // No same-day re-schedule: it already appeared today (markAppearedToday ran at spawn),
        // so the next appearance waits for TimeEngine's midnight rollover signal, not a local
    ...
    // Only arm today's appearance if it hasn't already happened today (reopening the app on the
    // same calendar day must not spawn it again) — spawn() itself re-checks this too, in case the
    // date rolled over while this timeout was pending.
    if (!hasAppearedToday()) scheduleSpawn(options.firstDelayMs || 6000);

    // Re-arm for the next calendar day if the app is left open across midnight, via the same
    // TimeEngine rollover signal every other daily-reset module uses (Section 4/38) — not a
