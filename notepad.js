...
//
// Styling Phase 3: Notepad already has bespoke CSS (folder chips, hanging rod-and-flag layout) —
// that's Phase 10 territory, not this pass. Only the genuinely shared primitives are hooked here:
// composer text/select inputs (global input/select/textarea styling, now unwired from their old
// hardcoded purple), the format toggle and save/add buttons, the Saved Notes folder/note list
// rows (.list-row), and empty states (.empty-state). The hanging-panel/folder layout classes
// (notepad-hanging-*) are left completely untouched for their own Phase 10 pass.
const Notepad = (function () {
  ...
  function composerHtml() {
    ...
    const checklistRows = draftChecklist.map(function (item, idx) {
      return '<div class="notepad-draft-item" data-idx="' + idx + '">' +
        ...
        '<button class="notepad-draft-item-remove btn-icon" data-idx="' + idx + '" title="Remove item">&#10006;</button>' +
      '</div>';
    }).join('');
    ...
    return '<div id="notepad-modal">' +
      ...
      '<div id="notepad-top-row">' +
        '<h3 class="modal-title">' + (editingNoteId ? 'Edit Note' : 'Notepad') + '</h3>' +
        '<button id="notepad-saved-btn" class="btn-secondary" title="Saved notes">&#128193; Saved Notes</button>' +
      '</div>' +
      ...
      '</div>' +
        ...
        (composerType === 'text'
          ...
          : '<div id="notepad-checklist-draft">' +
              checklistRows +
              '<button id="notepad-add-item-btn" class="btn-secondary">+ Add item</button>' +
            '</div>') +
      '</div>' +
  ...
  function beginEditNote(noteId) {
    ...
    renderComposer();
      ...
      // Not touched by Styling Phase 3 — bespoke layout, refined in its own Phase 10 pass.
  ...
  function savedFoldersHtml() {
    ...
    const rows = folders.map(function (f) {
      ...
      return '<button class="notepad-folder-row list-row list-row-compact" data-folder="' + esc(f) + '">' +
        '<span class="notepad-folder-name list-row-title">&#128193; ' + esc(f) + '</span>' +
        '<span class="notepad-folder-count badge">' + count + '</span>' +
      '</button>';
    }).join('');

    return '<div id="notepad-modal">' +
      '<div id="notepad-top-row">' +
        '<button id="notepad-back-btn" class="btn-secondary" title="Back to composer">&#8592; Back</button>' +
        '<h3 class="modal-title">Saved Notes</h3>' +
      '</div>' +
      '<div id="notepad-folder-list">' + (rows || '<div class="empty-state">No folders yet.</div>') + '</div>' +
    '</div>';
  }
    ...
    const rows = notes.length ? notes.map(function (n) {
      if (n.type === 'checklist') {
        const items = (n.checklistItems || []).map(function (it) {
         ...
         return '<div class="assistant-note-row list-row list-row-compact" data-id="' + n.id + '">' +
          ...
          '<div class="list-row-actions">' +
            ...
            '<button class="assistant-note-delete-btn btn-danger" data-id="' + n.id + '">Delete</button>' +
          '</div>' +
        '</div>';
      }
      return '<div class="assistant-note-row list-row list-row-compact" data-id="' + n.id + '">' +
        '<span class="assistant-note-text list-row-title">' + esc(n.text) + '</span>' +
        '<div class="list-row-actions">' +
          '<button class="notepad-favorite-btn' + (n.favorite ? ' notepad-favorite-active' : '') + '" data-id="' + n.id + '" title="Favourite">&#11088;</button>' +
          '<button class="assistant-note-delete-btn btn-danger" data-id="' + n.id + '">Delete</button>' +
        '</div>' +
      '</div>';
    }).join('') : '<div class="empty-state">No notes in this folder yet.</div>';

    return '<div id="notepad-modal">' +
      '<div id="notepad-top-row">' +
        '<button id="notepad-folders-back-btn" class="btn-secondary" title="Back to folders">&#8592; Folders</button>' +
        '<h3 class="modal-title">' + esc(folder) + '</h3>' +
      '</div>' +
