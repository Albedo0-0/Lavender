...
// Sum of questionsSolved across this date's session records (1.2) — the session-specific
  ...
  return {
    ...
    doItLater: doItLater,
    // Exposed for Itinerary's orchestrator (Section 18/late-start handling) so a late study-item
    // start can shift today's OTHER already-scheduled Planner sessions in sync with the
    // itinerary's own shiftMs (ItineraryData.applyItineraryShift), the same two-system-in-sync
    // approach the existing foldLag pattern already uses on normal item resolution. Purely an
    // additive export of an existing internal function — no change to what applyShift itself does.
