...
const Settings = (function () {
  ...
  function formHtml() {
    ...
    return (
      ...
      (cur.breakWallpaper
        ? '<img id="settings-wallpaper-preview" src="' + cur.breakWallpaper + '" alt="Break wallpaper preview">' +
      ...
      (cur.studyWallpapers || []).map(function (item, i) {
        ...
        return '<div class="settings-wallpaper-thumb">' +
          (isVideo
            ...
            : '<img src="' + url + '" alt="Study wallpaper preview">') +
  ...
  function save() {
    ...
    if (typeof MiscSound !== 'undefined') MiscSound.play('uiSave');
    Modal.close();
  }
...
function _isFs() { return !!(document.fullscreenElement || document.webkitFullscreenElement); }
    ...
    document.getElementById('settings-import-input').addEventListener('change', function (e) {
      ...
      reader.onload = function () {
        ...
        Modal.close();
        // Restored My World content packs are written to IndexedDB asynchronously — let that finish first.
        const reload = function () { location.reload(); };
        Promise.resolve(result.settled).then(reload, reload);
      };
