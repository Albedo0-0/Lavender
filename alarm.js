...
const Alarm = (function () {
  ...
  function openItineraryList() {
    ...
    document.querySelectorAll('.itinerary-template-items-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        if (typeof Itinerary !== 'undefined') Itinerary.openBuilder(btn.dataset.id, openItineraryList);
      });
    });
  ...
  // returnFn: optional callback used instead of openItineraryList() on Cancel/Save — lets a
  // caller other than Alarm's own Itineraries tab (e.g. the Phase 6 morning gate's "Create
  // Itinerary" action) come back to itself once the template is saved, with no page reload.
  // On save of a brand-new template, hands straight into the Phase 5 Itinerary Builder
  // (Itinerary.openBuilder) so "Create Itinerary" is one continuous flow, then that builder's
  // own Save/Cancel returns to backFn — matching Section 16's "one editor, two entry points".
  function openItineraryForm(id, returnFn) {
    ...
    document.getElementById('itinerary-template-cancel-btn').addEventListener('click', backFn);
    ...
    document.getElementById('itinerary-template-save-btn').addEventListener('click', function () {
      ...
      if (wasNew && typeof Itinerary !== 'undefined') { Itinerary.openBuilder(result.templateId, backFn); return; }
      backFn();
    });
  }
  ...
  return { init: init, openList: openList, openItineraryList: openItineraryList, openItineraryForm: openItineraryForm };
})();
