...
const MyWorldContent = (function () {
  ...
  // A tree definition specifies its own stage count via the length of its
  // own `stages` array — Core no longer fixes this to a single number.
  // MIN/MAX_PROCEDURAL_STAGES are just sanity bounds against malformed or
  // hostile definitions (e.g. an imported pack), not a target count. LV1's
  // built-in apple tree has 22 stages; a future LV's tree is free to define
  // any count in range (50, for instance) with no further Core change.
  const MIN_PROCEDURAL_STAGES = 2;
  const MAX_PROCEDURAL_STAGES = 500;
  ...
  function validateTree(d) {
    ...
    if (!Array.isArray(d.stages) || d.stages.length < MIN_PROCEDURAL_STAGES || d.stages.length > MAX_PROCEDURAL_STAGES) return 'stages';
  ...
  // A bare `new Function('LV', src)` still runs in the real global scope,
  // so an imported script could reach MyWorldData/MyWorldContent/MyWorld/
  // window/document/etc. simply by naming them — the sandbox was capability
  // in name only. These names are shadowed as extra parameters (bound to
  // `undefined`) on the function built for the script below, so a direct
  // reference resolves to nothing instead of the real object. `LV` is
  // deliberately not in this list — it's the one capability the script is
  // meant to receive.
  const SANDBOX_SHADOW_NAMES = [
    'window', 'self', 'globalThis', 'top', 'parent', 'frames', 'opener',
    'document', 'navigator', 'location', 'history', 'caches',
    'localStorage', 'sessionStorage', 'indexedDB',
    'fetch', 'XMLHttpRequest', 'WebSocket', 'Worker', 'SharedWorker',
    'Function', 'eval', 'importScripts',
    'MyWorld', 'MyWorldData', 'MyWorldContent', 'MyWorldAssets', 'MyWorldLV1',
    'Nav', 'Modal', 'State'
  ];
  ...
  function registerPack(api) {
    ...
    // A reserved id (e.g. the built-in 'lv1') may register itself once,
    // the first time; any further registration under that same id is
    // rejected rather than silently replacing it.
    if (packRegistry[api.id] && RESERVED_PACK_IDS.indexOf(api.id) >= 0) {
      warn('rejected pack registration: "' + api.id + '" is reserved and already registered');
      return false;
    }
   ...
   * assigning a window/page global. Every real global that could leak
   * Core internals, other LV state, or the page (window, document,
   * MyWorldData, MyWorldContent, MyWorldAssets, MyWorld, localStorage,
   * etc. — see SANDBOX_SHADOW_NAMES) is shadowed as an extra, unfilled
   * parameter, so a same-name reference inside the script resolves to
   * `undefined` rather than the real object. Nothing outside the `LV`
   * object is reachable by name.
   */
    ...
    return openPackDb().then(function (db) {
      ...
      try {
        const fn = new Function(
          'LV',
          SANDBOX_SHADOW_NAMES.join(', '),
          '"use strict";\n' + row.source
        );
        const api = fn.apply(null, [makePackSandbox(lvPackId)]);
  ...
  function restoreImportedPacks() {
    return loadAllPackDefs().then(function (defs) {
      ...
      return Promise.all(behaviourIds.map(loadBehaviour)).then(function () {
        // Boot race fix: MyWorld's initial applyContent() runs synchronously
        // at script load, before this (async, IndexedDB-backed) restore can
        // possibly finish. If the active world is an imported one, its pack
        // may not have been registered yet at that point, leaving the
        // active pack slot unresolved. Re-sync it now that every imported
        // behaviour pack is registered.
        if (typeof MyWorld !== 'undefined' && MyWorld && MyWorld.refreshActivePack) {
          MyWorld.refreshActivePack();
        }
        return defs.length;
      });
  ...
  function importPackFile(file) {
    return file.text().then(function (text) {
      ...
      if (pack.behaviour !== undefined) {
        ...
        // Reject a behaviour id that's already owned by a different,
        // previously-installed pack (registerPack collision guard) — but
        // still allow this same pack to be re-imported as an update, since
        // its own definitions legitimately reuse that id.
        const importedDefIds = pack.definitions.map(function (d) { return d && d.id; });
        const claimedByOther = order.some(function (oid) {
          return registry[oid] && registry[oid].type === 'world' &&
            registry[oid].lvPack === pack.behaviour.lvPackId &&
            importedDefIds.indexOf(oid) < 0;
        });
        if (claimedByOther) throw new Error('That pack id is already used by another installed pack.');
      }
