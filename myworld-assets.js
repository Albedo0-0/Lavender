...
const MyWorldAssets = (function () {
  ...
  function getStageByIndex(index) {
    if (typeof index !== 'number' || index < 0) return TREE_STAGES[0];
    if (index >= TREE_STAGES.length) return TREE_STAGES[TREE_STAGES.length - 1];
    return TREE_STAGES[index];
  }

  function getStageIndexForGrowthPoints(growthPoints) {
    ...
    for (let i = 0; i < TREE_STAGES.length; i++) {
      if (gp >= TREE_STAGES[i].minGrowthPoints) idx = i;
      else break;
    }
    return idx;
  }

  function getStageForGrowthPoints(growthPoints) {
    return getStageByIndex(getStageIndexForGrowthPoints(growthPoints));
  }

  /** 0..1 progress from the current stage's threshold toward the next stage's. */
  function getStageProgress(growthPoints) {
    const idx = getStageIndexForGrowthPoints(growthPoints);
  ...
  function isFinalStage(index) {
    return index >= TREE_STAGES.length - 1;
  }
