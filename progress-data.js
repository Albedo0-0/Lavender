...
const ProgressData = (function () {
  ...
  // §4.3 — hydration score already 0-10, computed by WaterData; Progress just reads it.
  function hydrationValue(entry, dateStr) {
    return (dateStr && typeof WaterData !== 'undefined') ? WaterData.getScoreForDate(dateStr) : null;
  }

  // §5.3 — sleep duration in hours, only for completed records.
  function sleepValue(entry, dateStr) {
    if (!dateStr || typeof SleepData === 'undefined') return null;
    const rec = SleepData.getRecord(dateStr);
    if (!rec || !rec.completed || rec.durationMin === null || rec.durationMin === undefined) return null;
    return Math.round((rec.durationMin / 60) * 10) / 10;
  }
