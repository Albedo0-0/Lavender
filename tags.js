...
const Tags = (function () {
  ...
  function render(topicId) {
    ...
    const chipsHtml = allTags.length === 0 ? '<p class="planner-empty">No tags yet.</p>' :
      allTags.map(function (tag) {
        const isOn = attachedIds.indexOf(tag.tagId) !== -1;
        return '<button class="tag-chip' + (isOn ? ' tag-chip-active' : '') + '" data-tag-id="' + tag.tagId + '" style="border-color:' + tag.color + ';background:' + (isOn ? tag.color : 'transparent') + '">' +
          esc(tag.name) +
    ...
    Modal.open(
      '<h3>Tags</h3>' +
      '<div class="tag-chip-list" id="tag-manager-list">' + chipsHtml + '</div>' +
      '<div class="tag-create-row">' +
        '<input type="text" id="tag-new-name" placeholder="New tag name">' +
        '<input type="color" id="tag-new-color" value="#b39ddb">' +
        '<button id="tag-new-save">Create</button>' +
      '</div>' +
      '<button id="tag-manager-close">Done</button>'
    );
